...
internal class AddressableUtils : Editor
{
    ...
    static AddressableMapData SetMapping(string dir, eAddressableType type)
    {
        ...
        foreach (var file in files)
        {
            ...
            data.assetType = 
            path.Contains(".png") ? eAssetType.sprite : 
            path.Contains(".json") ? eAssetType.jsondata : 
            path.Contains(".asset") ? eAssetType.tile : eAssetType.prefab;
    ...
    public async static Task DeleteFileinFolder(string type)
    {
        ...
        request.Credentials = new NetworkCredential("wocjf84", "afsd2977");
        ...
        catch (WebException ex)
        {
            ...
            switch (response.StatusCode)
            {
                case FtpStatusCode.ActionNotTakenFileUnavailable:
                    {
                        Debug.LogError("DeleteFolder ] Probably the folder already exist : ");
                    }
                    break;
            }
            response.Dispose();
    ...
    public async static Task CreateFolder(string type)
    {
        ...
        catch (WebException ex)
        {
            ...
            switch (response.StatusCode)
            {
                case FtpStatusCode.ActionNotTakenFileUnavailable:
                    {
                        Debug.LogError("CreateFolders ] Probably the folder already exist : ");
                    }
                    break;
            }
            response.Dispose();
